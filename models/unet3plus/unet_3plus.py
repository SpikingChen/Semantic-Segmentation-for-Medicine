...
class UNet3Plus(nn.Module):
    ...
    def __init__(self,
        ...
        self.decoder = Decoder(self.filters, self.cat_channels,
                               self.up_channels)
        if self.is_deepsup:
            self.deepsup = DeepSup(self.up_channels, self.filters, self.num_classes)
            if self.is_CGM:
    ...
    def forward(self, inputs):
        ...
        if self.is_deepsup:
            ...
            if self.is_CGM:
                ...
                cls_branch_max = cls_branch_max.view(-1, 1)
...
class UNet_3Plus_DeepSup_CGM(nn.Module):
    ...
    def __init__(self, in_channels=3, num_classes=1, feature_scale=4, is_deconv=True, is_batchnorm=True):
        ...
        # DeepSup
        self.outconv1 = nn.Conv2d(self.UpChannels, self.num_classes, 3, padding=1)
        self.outconv2 = nn.Conv2d(self.UpChannels, self.num_classes, 3, padding=1)
        self.outconv3 = nn.Conv2d(self.UpChannels, self.num_classes, 3, padding=1)
        self.outconv4 = nn.Conv2d(self.UpChannels, self.num_classes, 3, padding=1)
        self.outconv5 = nn.Conv2d(filters[4], self.num_classes, 3, padding=1)
