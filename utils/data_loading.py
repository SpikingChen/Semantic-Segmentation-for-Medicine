...
class OCTDataset(BasicDataset):
    def __init__(self, images_dir, masks_dir=None, new_size=512, type='train', data_aug=False):
    ...
    def preprocess_img_mask(self, pil_img, pil_mask):
        img_ndarray = np.asarray(pil_img).copy()
    ...
    def preprocess_train(self, pil_img, pil_mask):
        img_ndarray, mask_ndarray = self.preprocess_img_mask(pil_img, pil_mask)

        one_hot_ndarray = np.zeros((4, mask_ndarray.shape[0], mask_ndarray.shape[1])).astype('int64')
        one_hot_ndarray[0, mask_ndarray == 0] = 1
        one_hot_ndarray[1, mask_ndarray == 1] = 1
        one_hot_ndarray[2, mask_ndarray == 2] = 1
        one_hot_ndarray[3, mask_ndarray == 3] = 1
        ...
        def one_hot2dist(seg: np.ndarray) -> np.ndarray:
            ...
            res = np.zeros_like(seg, dtype=seg.dtype)
            for k in range(K):
                posmask = seg[k].astype(np.bool)

                if posmask.any():
                    negmask = ~posmask
                    res[k] = eucl_distance(negmask) * negmask - (eucl_distance(posmask) - 1) * posmask
                # The idea is to leave blank the negative classes
                # since this is one-hot encoded, another class will supervise that pixel

            return res

        dist_map_ndarray = one_hot2dist(one_hot_ndarray)

        return img_ndarray, mask_ndarray, dist_map_ndarray

    def preprocess_val(self, pil_img, pil_mask):
    ...
    def __getitem__(self, idx):
        if self.type in ['train', 'val']:
            ...
            if self.type == 'train':
                img, mask, dist_map = self.preprocess_train(img, mask)

                return {
                    'image': torch.as_tensor(img.copy()).float().contiguous(),
                    'mask': torch.as_tensor(mask.copy()).long().contiguous(),
                    'distmap': torch.as_tensor(dist_map.copy()).long().contiguous()
                }
            else:
                img, mask, dist_map = self.preprocess_val(img, mask)
                return {
                    'image': torch.as_tensor(img.copy()).float().contiguous(),
                    'mask': torch.as_tensor(mask.copy()).long().contiguous()
                }
